...
var Handlers = {
  ...
  OPEN: function (id, path, pflags, attrs) {
    var self = this;
    var flags =
      ((pflags & FXF_READ) ? O_RDONLY : 0) |
      ((pflags & FXF_WRITE) ? O_WRONLY : 0) |
      ((pflags & FXF_APPEND) ? O_APPEND : 0) |
      ((pflags & FXF_CREAT) ? O_CREAT : 0) |
      ((pflags & FXF_TRUNC) ? O_TRUNC : 0) |
      ((pflags & FXF_EXCL) ? O_EXCL : 0);
  ...
  SETSTAT: function (id, path, attrs) {
    ...
    if (attrs.permissions) {
      ...
      // TODO: Implement
      console.error("WARNING: SETSTAT is not fully implemented");
      self.status(id, FX_OK, "Success");
    }
  },
  FSETSTAT: function (id, handle, attrs) {
    // TODO: Implement
    console.error("WARNING: FSETSTAT is not implemented");
    this.status(id, FX_OP_UNSUPPORTED, "Unsupported");
  },
  ...
  SYMLINK: function(id, linkPath, targetPath) {
    var self = this;
  ...
  EXTENDED: function(/*TODO*/) {
    console.error("WARNING: EXTENDED is not implemented");
    this.status(id, FX_OP_UNSUPPORTED, "Unsupported");
  }
};
